...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, strides=1):
        ...
        self.block = nn.Sequential(
            ...
            nn.Conv2d(
                out_channels, out_channels, kernel_size=3, stride=strides, padding=1
            ),
            nn.LeakyReLU(inplace=True),
        )
        self.conv11 = nn.Conv2d(
            in_channels, out_channels, kernel_size=1, stride=strides, padding=0
        )
    ...
    def flops(self, H, W):
        flops = (
            H * W * self.in_channels * self.out_channels * (3 * 3 + 1)
...
class PosCNN(nn.Module):
    def __init__(self, in_channels, embed_dim=768, s=1):
        ...
        self.proj = nn.Sequential(
            nn.Conv2d(in_channels, embed_dim, 3, s, 1, bias=True, groups=embed_dim)
        )
        self.s = s
...
class SepConv2d(torch.nn.Module):
    def __init__(
        ...
        self.offset_conv1 = nn.Conv2d(
            in_channels, 216, 3, stride=1, padding=1, bias=False
        )
...
class Upsample_advanced(nn.Module):
    def __init__(self, in_channels, out_channels, factor):
        ...
        self.deconv = nn.Sequential(
            nn.ConvTranspose2d(in_channels, out_channels, kernel_size=2, stride=factor),
        )
        self.in_channels = in_channels
        self.out_channels = out_channels
...
class TRANSFORMER_BLOCK(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [
                Deformable_Attentive_Transformer(
                    dim=dim,
...
class AIDTransformer(nn.Module):
    # def __init__(self, img_size=64, in_channels=4,
    # def __init__(self, img_size=64, in_channels=7,
    def __init__(
        self,
        img_size=64,
        in_channels=182,
        ...
        **kwargs,
        ...
        self.output_proj = OutputProj(
            in_channels=3 * embed_dim, out_channels=in_channels, kernel_size=3, stride=1
        )
        self.output_proj1 = OutputProj(
            in_channels=embed_dim, out_channels=in_channels, kernel_size=3, stride=1
        )
        # Encoder
        self.encoderlayer_0 = TRANSFORMER_BLOCK(
            dim=embed_dim,
        ...
        self.encoderlayer_1 = TRANSFORMER_BLOCK(
            dim=embed_dim * 3,
        ...
        self.encoderlayer_2 = TRANSFORMER_BLOCK(
            dim=embed_dim * 5,
        ...
        self.encoderlayer_3 = TRANSFORMER_BLOCK(
            dim=embed_dim * 8,
        ...
        self.decoderlayer_0 = TRANSFORMER_BLOCK(
            dim=embed_dim * 14,
        ...
        self.decoderlayer_1 = TRANSFORMER_BLOCK(
            dim=embed_dim * 8,
            output_dim=embed_dim * 8,
            input_resolution=(img_size // (2**2), img_size // (2**2)),
            depth=depths[6],
        ...
        self.decoderlayer_2 = TRANSFORMER_BLOCK(
            dim=embed_dim * 3,
            output_dim=embed_dim * 4,
            input_resolution=(img_size // 2, img_size // 2),
            depth=depths[7],
        ...
        self.decoderlayer_3 = TRANSFORMER_BLOCK(
            dim=embed_dim * 2,
            output_dim=embed_dim * 2,
            input_resolution=(img_size, img_size),
            depth=depths[8],
    ...
    def forward(self, x, mask=None):
        ...
        pool2 = torch.cat(
            [pool2, self.hs_downsample(self.hs_downsample(self.hs_downsample(conv21)))],
            -1,
        )
        conv3 = self.conv(pool2, mask=mask)
        up0 = self.upsample_0(conv3)
        deconv0 = torch.cat(
            [up0, conv2, self.hs_downsample(self.hs_downsample(conv31))], -1
        )
