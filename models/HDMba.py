...
class MambaBlock(nn.Module):
    def __init__(self, seq_len, d_model, state_size, device):
        ...
        # 添加1D卷积
        self.conv = nn.Conv1d(
            seq_len, seq_len, kernel_size=3, padding=1, groups=seq_len, device=device
        )
        # 添加线性层
...
class Mamba(nn.Module):
    def __init__(self, seq_len, d_model, state_size, device):
        ...
        self.d_model = d_model
        self.state_size = state_size
        self.mamba_block1 = MambaBlock(
            self.seq_len, self.d_model, self.state_size, device
        )
        self.mamba_block2 = MambaBlock(
            self.seq_len, self.d_model, self.state_size, device
        )
        self.mamba_block3 = MambaBlock(
            self.seq_len, self.d_model, self.state_size, device
        )
...
class SSMamba(nn.Module):
    def __init__(
        ...
        self.mamba = Mamba(
            seq_len=self.window_size**2, d_model=dim, state_size=dim, device=device
        )
    ...
    def forward(self, x):
        ...
        shortcut = x
        x = (
            x.permute(0, 2, 3, 1).contiguous().view(B, H * W, C)
        ...
        x = self.norm2(x)
        x = (
            x.view(B, H, W, C).permute(0, 3, 1, 2).contiguous()
        )  # Change shape back to (B, C, H, W)

        x = shortcut + self.drop_path(x)
        x = x + self.drop_path(
            self.mlp(x.permute(0, 2, 3, 1).contiguous().view(B, H * W, C))
            .view(B, H, W, C)
            .permute(0, 3, 1, 2)
            .contiguous()
        )
...
class SSMaBlock(nn.Module):
    def __init__(self, dim=32, window_size=8, depth=4, mlp_ratio=2, drop_path=0.0):
