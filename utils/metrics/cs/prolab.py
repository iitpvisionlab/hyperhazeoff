...
class ProLab:
    ...
    def from_XYZ(self, color: Tensor) -> Tensor:
        illuminant_xyz = self._illuminant_xyz.view(-1, *((1,) * (color.dim() - 1))).to(
            device=color.device
        )
        color_ = color / illuminant_xyz
        return (torch.tensordot(self.Q.to(device=color.device), color_, dims=1)) / (
            torch.tensordot(self.q.to(device=color.device), color_, dims=1) + 1.0
        )

    def to_XYZ(self, color: Tensor) -> Tensor:
        y2 = torch.tensordot(self.Q_inv, color, dims=1)
        xyz = y2 / (1.0 - torch.tensordot(self.q.to(device=color.device), y2, dims=1))
        return xyz * self._illuminant_xyz.view(-1, *((1,) * (color.dim() - 1))).to(
            device=color.device
        )
